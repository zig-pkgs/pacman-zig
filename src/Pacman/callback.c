...
static void fill_progress(config_t *_config, const int percent, const int proglen)
{
...
void cb_event(void *ctx, alpm_event_t *event)
{
	...
	switch(event->type) {
		...
		case ALPM_EVENT_PKG_RETRIEVE_START:
			...
			total_enabled = list_total && list_total_pkgs > 1 && dload_progressbar_enabled(_config);

			if(total_enabled) {
				init_total_progressbar(_config);
			}
			break;
...
{
	config_t *_config = ctx;
	const colstr_t *colstr = &_config->colstr;
		...
		int percent, size_t howmany, size_t current)
{
	...
	if(_config->noprogressbar || cols == 0) {
		return;
	}
	...
	fill_progress(_config, percent, cols - infolen);
...
int dload_progressbar_enabled(config_t *_config)
{
	return !_config->noprogressbar && (getcols() != 0);
}
...
void init_total_progressbar(config_t *_config)
{
...
void draw_pacman_progress_bar(struct pacman_progress_bar *bar)
{
	...
	fill_progress(bar->config, file_percent, cols - infolen);
	return;
}
...
void dload_init_event(config_t *_config, const char *filename, alpm_download_event_init_t *data)
{
	...
	if(!dload_progressbar_enabled(_config)) {
...
void cb_download(void *ctx, const char *filename, alpm_download_event_type_t event, void *data)
{
	...
	if(event == ALPM_DOWNLOAD_INIT) {
		dload_init_event(_config, filename, data);
	} else if(event == ALPM_DOWNLOAD_PROGRESS) {
		dload_progress_event(_config, filename, data);
	} else if(event == ALPM_DOWNLOAD_RETRY) {
		dload_retry_event(_config, filename, data);
	} else if(event == ALPM_DOWNLOAD_COMPLETED) {
		dload_complete_event(_config, filename, data);
	} else {
