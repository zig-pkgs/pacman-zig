...
fn dload_init_event(filename: []const u8, data: [*c]c.alpm_download_event_init_t) !void {
    _ = data;
    const cleaned_filename = cleanFilename(filename);
    if (c.dload_progressbar_enabled() == 0) {
...
pub fn cb_download(
    ...
    switch (event) {
        c.ALPM_DOWNLOAD_INIT => c.dload_init_event(filename, @ptrCast(@alignCast(data))),
        c.ALPM_DOWNLOAD_PROGRESS => c.dload_progress_event(filename, @ptrCast(@alignCast(data))),
        c.ALPM_DOWNLOAD_RETRY => c.dload_retry_event(filename, @ptrCast(@alignCast(data))),
        c.ALPM_DOWNLOAD_COMPLETED => c.dload_complete_event(filename, @ptrCast(@alignCast(data))),
