...
pub fn colonPrint(comptime format: []const u8, args: anytype) void {
    if (std.io.getStdErr().isTty()) {
        const prefix = Config.color_str.On.colon;
        const postfix = Config.color_str.On.nocolor;
        print(prefix ++ format ++ postfix, args);
    } else {
        const prefix = Config.color_str.Off.colon;
        const postfix = Config.color_str.Off.nocolor;
        print(prefix ++ format ++ postfix, args);
    }
}
...
pub const SoftInterrupt = struct {
    ...
    pub fn removeHandler() void {
        ...
        posix.close(self_pipe_fds[0]);
        posix.close(self_pipe_fds[1]);
    }
